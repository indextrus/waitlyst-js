...
export class Waitlyst {
  ...
  private generateElements() {
    ...
    const elements = document.querySelectorAll("[data-waitlyst=feedback]");
  ...
  /* Alias an event - returns a Promise */
  public alias(newId: string): Promise<any> {
    return this.eventManager.alias(newId);
  }

  /* Group event - returns a Promise */
  public group(groupId: string, traits?: any): Promise<any> {
    return this.eventManager.group(groupId, traits);
  }

  /* Screen event - returns a Promise */
  public screen(name: string, properties?: any): Promise<any> {
    return this.eventManager.screen(name, properties);
  }
}
